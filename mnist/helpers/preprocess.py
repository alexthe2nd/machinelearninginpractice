...
class PreProcesser():
    ...
    def augment_mnist_data(dataset, augmenter, augmented_ratio=1):
        ...
        for i in tqdm(range(augmented_ratio * training_length),
                      desc="Augmenting images", unit="image"):
            # Augment images
            train_images.append(np.reshape(augmenter.augment_image(
                dataset.train.images[i % training_length].reshape(28, 28, 1)),
                                           784))
            if i==0:
...
if __name__ == '__main__':
    ...
    aug_dataset = PreProcesser.augment_mnist_data(dataset, augmenter=augmenter,
                                                  augmented_ratio=2)
    PreProcesser.save(aug_dataset, '../Augmented_MNIST_data/saved_dataset_1.pkl')
